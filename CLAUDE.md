...
- **`Movie/`** — camera + effects systems: `CameraPath`/`CamMarkers`/`CamPathEval`/`CamPlayback` (BO2-style dolly + timeline playback), `FollowCamera` + `FollowTarget*`/`FollowEventIndex`, `MovieMode`, `ViewFx`/`ViewFxVm` (camera-feel modifiers), `BodyCam`/`ActionCam` (Follow presets), `DemoEndHold`, and the **ParticleFx subsystem** split across `ParticleFx.cpp` (core/dispatch), `ParticleFxRules` (swap tables), `ParticleFxHook` (detour), `ParticleFxSpray`, `ParticleFxMoney`, `ParticleFxSettings`, `ParticleFxDiagnostics` behind `ParticleFx.h` (+ `ParticleFxInternal.h`).
